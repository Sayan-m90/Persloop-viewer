...
{
  int dimensions, noPoints, currentEdgeSize = 0, barcode_count = 0, dimension = 3;


  string filtration_file, simpers_file;
  
  ParseCommand(argc, argv, dimension, simpers_file, filtration_file);
  ...
  std::vector<bg::model::point<float, dimension, bg::cs::cartesian>> vPoint;
...
//    getchar();
  ...
  {     
    bg::model::point<float, dimension, bg::cs::cartesian> pointbuff;
      for(auto midim=0;midim<dimension;midim++){
          float bc;
          ff>>bc;
          pc.set<midim>(bc);
      }
